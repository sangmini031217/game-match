...
// ── OW HERO NAMES BY ROLE ──
const OW_HERO_NAMES={'탱커':['라인하르트','윈스턴','디바','레킹볼'],'딜러':['트레이서','겐지','위도우메이커','솔저'],'힐러':['메르시','아나','루시우','모이라'],'자유':['라인하르트','트레이서','메르시','겐지','디바','아나','솔저','윈스턴','루시우','위도우메이커','모이라','레킹볼']};
function getNextNick(role){
  const pool=OW_HERO_NAMES[role]||OW_HERO_NAMES['자유'];
  const available=pool.filter(h=>!players.some(p=>p.name===h));
  if(available.length)return available[nickIdx++%available.length];
  return pool[nickIdx++%pool.length]+(nickIdx);
}
...
function openModal(){
  ...
  if(autoName){
    // auto name: skip name step, go straight to tier
    const steps=simpleMode?2:3;
    $('stepIndicator').innerHTML=Array.from({length:steps},(_,i)=>`<div class="step-dot" id="dot${i+1}"></div>`).join('');
    buildTierChips();goStep(2);
  } else {
    const steps=simpleMode?3:4;
    $('stepIndicator').innerHTML=Array.from({length:steps},(_,i)=>`<div class="step-dot" id="dot${i+1}"></div>`).join('');
    goStep(1);$('inputName').value='';$('inputName').placeholder=ot('inputPlaceholder');
  }
...
function selRole(el,r){draft.role=r;$('roleChips').querySelectorAll('.chip').forEach(c=>c.classList.remove('selected'));el.classList.add('selected');setTimeout(()=>{
  const autoName=$('autoNameToggle').checked;
  if(autoName&&!draft.name){draft.name=getNextNick(r);}
  finishAdd();
},180);}
